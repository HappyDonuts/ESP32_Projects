...
ssd1306_t* ssd1306_new(uint8_t addr){
	ssd1306_t* ssd1306 = malloc(sizeof(*ssd1306));
	SSD1306_Init(ssd1306, addr);
	return ssd1306;
}

uint8_t SSD1306_Init(ssd1306_t* ssd1306, uint8_t addr) {
...
void ssd1306_I2C_WriteMulti(ssd1306_t* ssd1306, uint8_t reg, uint8_t* data, uint16_t count) {
	...
	i2c_master_write_to_device(0, ssd1306->addr, dt, count, 1000 / portTICK_PERIOD_MS);
}
...
void ssd1306_I2C_Write(ssd1306_t* ssd1306, uint8_t reg, uint8_t data) {
	...
	i2c_master_write_to_device(0, ssd1306->addr, dt, 2, 1000 / portTICK_PERIOD_MS);
